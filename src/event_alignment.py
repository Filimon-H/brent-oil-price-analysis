...
def summarize_model_impact(trace, change_date: pd.Timestamp, closest_event: pd.Series) -> str:
    """
    Summarizes model parameter changes and matched event using raw column names.

    Args:
        trace (arviz.InferenceData): PyMC posterior trace
        change_date (pd.Timestamp): Detected change point date
        closest_event (pd.Series): Closest event info with original column names
...
Matched Event: {closest_event['event_name']} ({closest_event['start_date'].date()})
